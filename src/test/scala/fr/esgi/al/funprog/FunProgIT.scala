...
class FunProgIT extends AnyFunSpec {
  val funProg: FunProg = FunProg(GetPointsOfOthers(), StartGivenLawnMower())
  ...
  describe("given limit, inputs and output provided by statement") {
    ...
    val expectFirstLawnMower = firstLawnMower.copy(end = (
      Point(x = 1, y = 3),
      Direction.N
    ))
    val expectSecondLawnMower = secondLawnMower.copy(end = (
      Point(x = 5, y = 1),
      Direction.E
    ))
    ...
    describe("first lawn mower") {
      ...
      describe("second lawn mower") {
        ...
        it("should return expected result") {
          ...
          val expectedFirstLawnMower = firstLawnMower.copy(end = (Point(x = 0, y = 1), Direction.E))
          val expectedSecondLawnMower = secondLawnMower.copy(end = (Point(x = 1, y = 1), Direction.W))
